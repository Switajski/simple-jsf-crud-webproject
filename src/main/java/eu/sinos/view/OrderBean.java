...
public class OrderBean implements Serializable {
	...
	@PersistenceContext(unitName = "webproject-persistence-unit", type = PersistenceContextType.EXTENDED)
	...
	private Predicate[] getSearchPredicates(Root<Order> root) {
		...
		String articleID = this.example.getArticleID();
		if (articleID != null && !"".equals(articleID)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("articleID")),
					'%' + articleID.toLowerCase() + '%'));
		}
		String articleName = this.example.getArticleName();
		if (articleName != null && !"".equals(articleName)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("articleName")),
					'%' + articleName.toLowerCase() + '%'));
		}
		Long userID = this.example.getUserID();
		if (userID != null && userID.intValue() != 0) {
			predicatesList.add(builder.equal(root.get("userID"), userID));
		}
		Integer amount = this.example.getAmount();
		if (amount != null && amount.intValue() != 0) {
			predicatesList.add(builder.equal(root.get("amount"), amount));
		}
